...
class MoodleActivityMetadata(BaseModel):
    ...
    type: LearningResourceType = Field(..., description="Typ der Moodle-Aktivität")
    module_name: str = Field(..., description="Name des Moduls")
    intro: Optional[str] = Field(None, description="Beschreibung des Moduls")
...
class MoodleExtractedData(BaseModel):
    ...
    required_plugins: Dict[str, Set[str]] = Field(default_factory=dict)
    ai_results: Dict[str, Optional[object]] = Field(default_factory=dict, description="Automatisch generierte AI analyse")
