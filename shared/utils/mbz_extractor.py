...
class MBZExtractionResult:
    ...
    required_plugins: List[Tuple[str, str]] = None
...
class MBZExtractor:
    ...
    def extract_mbz(self, mbz_path: Path) -> MBZExtractionResult:
        ...
        if result.moodle_backup_xml and result.moodle_backup_xml.exists():
            result.required_plugins = get_required_plugins(result.moodle_backup_xml)
...
def get_required_plugins(moodle_backup_xml: Path) -> Dict[str, Set[str]]:
    """Extract activity and block plugins from moodle_backup.xml"""
    plugins = {
        "activities": set(),
        "blocks": set(),
        "filters": set(),
    ...
    try:
        tree = ET.parse(moodle_backup_xml)
        root = tree.getroot()


        # Activities
        for activity in root.findall("./information/contents/activities/activity"):
            name = activity.findtext("modulename")
            if name:
                plugins["activities"].add(name.strip())

        # Blocks
        for block in root.findall("./information/contents/blocks/block"):
            name = block.findtext("blockname")
            if name:
                plugins["blocks"].add(name.strip())

        # Content Bank (H5P etc.)
        for setting in root.findall(".//setting"):
            name = setting.findtext("name")
            value = setting.findtext("value")
            if name == "contentbankcontent" and value == "1":
                plugins["contentbank"].add("contentbank")

        all_unique_plugins = set().union(
            plugins["activities"],
            plugins["blocks"],
            plugins["filters"],
            plugins["contentbank"]
        )
        plugins["plugincounter"] = len(all_unique_plugins)

    except Exception as e:
        print(f"⚠️ Fehler beim Parsen von moodle_backup.xml: {e}")
